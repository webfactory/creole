<?php
    ...
    ini_set('include_path', CREOLE_TEST_BASE.DIRECTORY_SEPARATOR.'classes'.
                            PATH_SEPARATOR.CREOLE_TEST_BASE.'/../classes'.
    ...
    */
    ...
    set_error_handler(['CreoleBaseTest', 'errorHandler']);

    // if no DSN was supplied, then we need to prompt for it
    if ($argc <= 1) {
        echo "\n";
        echo "You must provide connection information to a database in order to test\n";
        echo "driver classes.  The database should be temporary or disposable.  The \n";
        echo "unit tests will create and drop tables within the specified database.\n";
        echo "\nYou can also specify this on the commmandline:\n";
        echo "\t$> php -f run-tests.php mysql://root@localhost/mytempdb\n";
    ...
    echo "[It is safe to ignore any errors related to dropping nonexistant tables.]\n\n";

    try {
        DriverTestManager::addSuite($testSuite, $dsn);
        PHPUnit2_TextUI_TestRunner::run($testSuite);
    } catch (Exception $e) {
        echo 'Could not add suite for '.$dsn.': '.$e->getMessage();
    }
