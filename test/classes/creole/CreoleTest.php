...
class CreoleTest extends CreoleBaseTest
{
    ...
    {
        $dsn = 'phptype://username:password@hostspec:110//usr/db_file.db?param1=value1&param2=value2';
        $dsninfo = Creole::parseDSN($dsn);

        $this->checkDSN($dsninfo, ['phptype', 'username', 'password', 'hostspec', 'port' => '110',
                                         'database' => '/usr/db_file.db', 'param1' => 'value1', 'param2' => 'value2',
                            ]);

        $dsn = 'phptype:///path/to/dbfile.db';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'database' => '/path/to/dbfile.db']);

        $dsn = 'phptype://username:password@hostspec/C:\path\to\dbfile.db';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'username', 'password', 'hostspec', 'database' => 'C:\path\to\dbfile.db']);

        $dsn = 'phptype://username:password@hostspec/database';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'username', 'password', 'hostspec', 'database']);

        $dsn = 'phptype://username:password@hostspec';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'username', 'password', 'hostspec']);

        $dsn = 'phptype://username@hostspec';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'username', 'hostspec']);

        $dsn = 'phptype://hostspec/database';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'hostspec', 'database']);

        $dsn = 'phptype://protocol(hostspec)/database';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype', 'hostspec', 'protocol', 'database']);

        $dsn = 'phptype';
        $this->checkDSN(Creole::parseDSN($dsn), ['phptype']);
    }
     ...
     * @test
     */
    public function registerCatchallDriver()
    {
     ...
     * @test
     */
    public function unregisterDriver()
    {
