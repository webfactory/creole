...
class ResultSetTest extends CreoleBaseTest
{
     ...
     * @test
     */
    public function fetchmodeAssoc()
    {
        $exch = DriverTestManager::getExchange('ResultSetTest.ALL_RECORDS');
     ...
     * @test
     */
    public function limitScrollBackwards()
    {
        $exch = DriverTestManager::getExchange('ResultSetTest.ALL_RECORDS');
        $stmt = $this->conn->createStatement();
        $stmt->setLimit(10);
    ...
    public function getBoolean()
    {
        ...
        $rs = $this->conn->executeQuery($exch->getSql(), ResultSet::FETCHMODE_NUM);
        $rs->next();
        $this->assertTrue(false === $rs->getBoolean(1), 'Expected answer to be false, was: '.$rs->getBoolean(1));
        ...
        $rs = $this->conn->executeQuery($exch->getSql(), ResultSet::FETCHMODE_NUM);
        $rs->next();
        $this->assertTrue(true === $rs->getBoolean(1));
        try {
