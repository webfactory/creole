...
class DriverTestManager
{
    ...
    protected static $driverClasses = ['Connection', 'ResultSet', 'PreparedStatement', 'CallableStatement', 'IdGenerator', 'Statement', 'DatabaseInfo', 'TableInfo'];
    ...
    public static function getExchange($id)
    {
        ...
        if (!$matches) {
            echo "XPath query matched no nodes: /exchanges/exchange[@id='".$id."']";
        ...
        if (is_array($sqlNodes)) {
            $sqlNode = $sqlNodes[0];
        } else {
            $sqlNode = $sqlNodes->item(0);
        }
